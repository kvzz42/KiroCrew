...
class AutoNudgeService:
        ...
        for raw in data.get("loops", []):
            try:
                ...
                if scheduled_at > 0 and loop.monitor is not None:
                    ...
                    # The requested instant is the authority until the single
                    # delivery is charged. Repair derivative fields rather than
                    # allowing a hand-edited interval/cap to change one-shot
                    # semantics after restart.
                    ...
                    if loop.gate:
                        loop.gate = False
                        scheduled_repaired = True
                    if loop.cycle_count == 0 and loop.next_due_ts != scheduled_at:
    ...
    def _arm_from_deadline(self, loop: NudgeLoop) -> None:
        ...
        if monitor is not None and monitor.version != MONITOR_STATE_VERSION:
            ...
            return
        # A successful scheduled dispatch keeps owning the session until its turn
        # completes. The completion hook reaches here with the one cycle charged;
        # run the ordinary cap gate immediately so it removes the one-shot row.
        # The same branch cleans a delivered row recovered after a crash, without
        # handing it a second fire.
        if is_scheduled_message(loop) and loop.cycle_count >= 1:
        ...
        if remaining <= 0:
            ...
            # An absolute schedule may be farther away than the recurring loop's
            # 24-hour interval ceiling. Sleeping to the deadline is safe because
            # user input cancels this task and restart recovery rebuilds it from
    ...
    async def _timer(self, loop: NudgeLoop, delay: float | None = None) -> None:
        ...
        if loop.max_cycles and loop.cycle_count >= loop.max_cycles:
            if is_scheduled_message(loop):
                # A delivered scheduled message is complete, not a stopped goal
                # that needs inspection. Removing it frees the session's single
                # automation slot for another schedule or Set a Goal. This path is
                # reached from turn completion and after restart, so the one cycle
                # stays exclusive through the turn and cannot be replayed.
                logger.info("AutoNudge: scheduled message %s delivered — removing", loop.id)
                await self.remove(loop.id)
                return
