...
while `sounds` (a synthesized per-state preset) stays legal on every tier for as
long as the shipped renderer reads a crew-record cue kind-agnostically. `expressions` (per-state eyes/mouth) round-trips on every tier too, for as long
as the shipped builder submits it and the shipped renderer draws it. A `motions` pick named on a
