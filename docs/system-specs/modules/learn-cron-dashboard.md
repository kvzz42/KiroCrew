...
Scheduled job execution with three schedule types: `every` (interval, min 60s), `at` (one-shot timestamp), `cron` (5-field expression). Supports natural language parsing via `parse_wakeup()`. The dashboard list exposes `at_ts` and `delete_after_run` alongside the recurring machine fields, so the Schedule editor represents an at-job as **Run once** rather than parsing its localized display string. PATCH accepts the same `at`/`delay`/`at_time` spellings as create; a same-kind retime preserves deletion-on-run, while converting to a recurring kind clears it. Newly created one-shots default to `strict_schedule=true` because they name an exact instant; an explicit false remains supported.
