...
| Files in chat | Browse, attach and upload workspace files; search the rail by file NAME or by file CONTENTS, including the text inside Word/PowerPoint/Excel documents; preview one in the side panel — a .docx renders its headings, lists, tables and bold/italic runs, falling back to extracted plaintext | Chat right panel → Files tab, Name/Content toggle below the search field; composer attach | `pages/chat/FileBrowserRail.tsx`, `pages/chat/FilesHomePanel.tsx`, `api/fileGrep.ts`, `components/FileRenderers.tsx` | `handlers/files.py` | `GET /api/file-read`, `GET /api/file-office-preview`, `POST /api/file-grep`, `POST /api/upload`, `POST /api/upload/file` |
...
| Scheduled chat message | One deferred composer message delivered as an ordinary user turn in its original persistent session; shares that session's single goal/monitor automation slot | Chat composer → **+** → **Send later…**; pending-message banner → editor | `components/ChatInput.tsx`, `components/ScheduleLaterPopover.tsx`, `components/SessionAutomationPopover.tsx` | `handlers/autonudge.py` | `POST /api/autonudge` (`at`), `PATCH,DELETE /api/autonudge/{loop_id}` |
